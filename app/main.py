...
def preprocess_data(apartment):
    ...
    # district_freq (Count Encoding) - districtin sayım nisbətini təyin edirik
    district_freq = district_count.get(apartment.district, 0)  # Default 0 if not in dictionary

    # district_target_enc (Target Encoding) - districtin qiymətini təyin edirik
...
def predict_price(apartment: Apartment):
    try:
        # Preprocessing tətbiq edirik
        features = preprocess_data(apartment)

        # Modeldən nəticə alırıq
